...
export default {
    ...
    computed: {
        // To know if your updating or editing in the Announcment Form
        editing() {
...
<template>
    <v-dialog width="1400px" v-model="dialog" persistent>
        <v-card class="d-flex justify-start items-center ga-1 pa-10 pb-5" style="border-radius: 1rem;">
            <!-- Title Section: Display different titles based on the action prop -->
            <h3 class="w-full text-center py-5 text-2xl font-extrabold">
                {{ editing ? 'UPDATE ANNOUNCEMENT' : 'NEW ANNOUNCEMENT' }}
                ...
                <div class="d-flex flex-col justify-center items-center gap-5 h-auto w-full col-span-1">    
                    ...
                    <!-- WHEN -->
                        ...
                        <v-card class="w-full" v-if="announcementInfo.datetimes && announcementInfo.datetimes.length > 0">
                            ...
                            <v-btn 
                                v-if="announcementInfo.datetimes.length > 0"
                                icon 
                                size="small" 
                                color="error" 
